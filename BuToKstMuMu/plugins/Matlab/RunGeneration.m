...
% Global variables %
%%%%%%%%%%%%%%%%%%%%
nFiles   = 100; % Number of files to generate [initially fixed 10]
nBins    =  2; % Number of q^2 bins [9]
startBin =  9; % Start bin [1...nBins]
...
% fidINval    = fopen('../../efficiency/ThetaKThetaL_B0ToKstMuMu.txt','r');

    fidINval    = fopen('../../efficiency/Bu2kstmm/ThetaKThetaL_BuToKstMuMu.txt','r');

% fidINcov    = fopen('../../efficiency/ThetaKThetaLPhiFullCovariance_B0ToKstMuMu_B0ToJPsiKst_B0ToPsi2SKst.txt','r');
% fidINcov    = fopen('../../efficiency/ThetaKThetaLFullCovariance_B0ToKstMuMu.txt','r');

    fidINcov    = fopen('../../efficiency/Bu2kstmm/covmat_cosThetaLcosThetaK.txt','r');

% fileNameOut = sprintf('../../efficiency/EffRndGenAnalyFilesSign_JPsi_Psi2S/Efficiency_RndGen_%d.txt',i-1);
% fileNameOut = sprintf('../../efficiency/Efficiency4D_B0ToKstMuMu_GoodTag_nsahoo_%d.txt',i-1);

    fileNameOut = sprintf('../../efficiency/EffRndmGen_BuToKstMuMu/Efficiency_RndGen_%d.txt',i-1);
   ...
   % fprintf(fidOUT,'%d\n',nBins*NcoeffThetaL+NcoeffPhi);
	...
	%{
	  ...
	  %}
        if (NcoeffPhi == 0 && EvalEffFunc2D(meanVOrig,showPlot) == false)
            fprintf('@@@ The original efficiency is negative @@@\n');
        else
            fprintf('@@@ The original efficiency is OK @@@\n');
        end
     end
